...
import { getPieceAt, isWallAt } from '../logic/chessLogic'; // Assuming pieces have unique IDs now
import { v4 as uuidv4 } from 'uuid'; // Need uuid library
import { findPath } from './pathfinding'; // Import BFS pathfinder
...
export function createRandomVisitor(initialRow, initialCol) {
    ...
    return {
        ...
        isChild: isChild, // Store if it's a child
        currentComment: null,
        stateTimer: Date.now() + Math.random() * 1000, // Start deciding soon
        commentTimer: null,
        currentPath: null, // Initialize path
    };
}
 ...
 * TODO: Optimize this if pieces array is large. Maybe a map?
...
function isOccupied(row, col, walls, pieces, visitors, selfId = null) {
    // Check walls and pieces at the exact grid location
    if (isWallAt(row, col, walls) || getPieceAt(row, col, pieces)) {
        return true;
    }
    // Check collision with other visitors at the exact grid location
...
export function updateVisitor(visitor, allPieces, walls, allVisitors) {
    ...
    if (updatedVisitor.state === VisitorState.MOVING_TO_TARGET) {
        if (!updatedVisitor.currentPath || updatedVisitor.currentPath.length === 0) {
            ...
            // Check if the *very next* cell is currently free (re-check needed due to other visitors)
            if (!isOccupied(nextCell.row, nextCell.col, walls, allPieces, allVisitors, updatedVisitor.id)) {
                ...
                updatedVisitor.currentPath.shift(); // Remove the step we just took
                 // If path now empty, we just arrived
                ...
                // Path blocked by someone who moved!
                console.log(`Visitor ${updatedVisitor.id} path blocked at ${nextCell.row},${nextCell.col}, recalculating...`);
                updatedVisitor.movementTarget = null;
                updatedVisitor.currentPath = null;
                updatedVisitor.state = VisitorState.IDLE; // Rethink plan
                // Increase delay significantly before retrying pathfinding
                updatedVisitor.stateTimer = now + 2000 + Math.random() * 3000; 
            }
        }
        return updatedVisitor; // Movement handled, skip timer check for state change
    } 
         ...
         if (!updatedVisitor.currentPath || updatedVisitor.currentPath.length === 0) {
            ...
            if (!isOccupied(nextCell.row, nextCell.col, walls, allPieces, allVisitors, updatedVisitor.id)) {
                ...
                // If path now empty, we just arrived at wander spot
                 if (updatedVisitor.currentPath.length === 0) {
                    updatedVisitor.movementTarget = null;
                    updatedVisitor.state = VisitorState.IDLE;
                    updatedVisitor.stateTimer = now + MIN_STATE_DURATION + Math.random() * (MAX_STATE_DURATION - MIN_STATE_DURATION);
                ...
                updatedVisitor.currentPath = null;
                updatedVisitor.state = VisitorState.IDLE; 
                // Increase delay significantly before retrying wander pathfinding
                updatedVisitor.stateTimer = now + 2000 + Math.random() * 3000; 
            }
        }
        return updatedVisitor; // Movement handled
    }

    // --- Timer-based State --- 
    if (now < updatedVisitor.stateTimer) {
        // Not time to change IDLE, OBSERVING, COMMENTING states
        return updatedVisitor; // Keep current state until timer expires
    }

    // Timer expired, evaluate state change for non-movement states
    switch (updatedVisitor.state) {
        case VisitorState.IDLE:
        case VisitorState.SELECTING_TARGET: // Combine Idle and Selecting Target
